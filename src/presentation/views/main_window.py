...
class MainWindow(QMainWindow):
    ...
    def _download_update(self, download_url: str, release_tag: str):
        """Download update"""

        safe_tag = re.sub(r'[^A-Za-z0-9._-]+', '_', release_tag or "latest")
        output_path = str(Path(tempfile.gettempdir()) / f"cali_sae_update_{safe_tag}.exe")
        ...
        try:
            ...
            if success:
                ...
                self._apply_update(Path(output_path), release_tag)
            else:
    ...
    def _apply_update(self, downloaded_exe: Path, release_tag: str):
        if not getattr(sys, "frozen", False):
            QMessageBox.information(self, "Éxito", f"Actualización descargada en:\n{downloaded_exe}")
            return

        current_exe = Path(sys.executable)
        if not downloaded_exe.exists():
            QMessageBox.warning(self, "Error", "No se encontró el archivo descargado para actualizar.")
            return

        state_path = self.main_controller.get_update_state_path()
        state_ps = ""
        if state_path and release_tag:
            safe_tag = release_tag.replace("'", "''")
            safe_path = state_path.replace("'", "''")
            state_ps = f"powershell -NoProfile -ExecutionPolicy Bypass -Command \"Set-Content -Path '{safe_path}' -Value '{{\\\"installed_tag\\\":\\\"{safe_tag}\\\"}}' -Encoding UTF8\""

        updater_bat = Path(tempfile.gettempdir()) / "cali_sae_apply_update.bat"
        lines = [
            "@echo off",
            "setlocal",
            f"set TARGET={str(current_exe)}",
            f"set UPDATE={str(downloaded_exe)}",
            ":loop",
        ...
        if state_ps:
            lines.append(state_ps)
        lines.extend([
            "start \"\" \"%TARGET%\"",
        ])
        content = "\r\n".join(lines) + "\r\n"
        updater_bat.write_text(content, encoding="utf-8")

        try:
            subprocess.Popen(["cmd.exe", "/c", str(updater_bat)], creationflags=subprocess.CREATE_NO_WINDOW)
        except Exception:
            subprocess.Popen(["cmd.exe", "/c", str(updater_bat)])

        QMessageBox.information(self, "Actualización", "La aplicación se cerrará para aplicar la actualización.")
        self.close()
