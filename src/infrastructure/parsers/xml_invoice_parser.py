...
class XMLInvoiceParser:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                for xml_file in xml_files:
                    try:
                        ...
                        invoice = self.parse_xml_content(xml_content, xml_file, Path(zip_path).name)
    ...
    def parse_xml_file(self, xml_path: str) -> Optional[Invoice]:
        ...
        try:
            ...
            return self.parse_xml_content(content, Path(xml_path).name, Path(xml_path).parent.name)
    ...
    def parse_xml_content(
        ...
        try:
            ...
            supplier = root.find(".//cac:AccountingSupplierParty/cac:Party", self.namespaces)
            seller_nit = ""
            seller_name = ""
            seller_municipality = ""
            ...
            customer = root.find(".//cac:AccountingCustomerParty/cac:Party", self.namespaces)
            buyer_nit = ""
            buyer_name = ""
    ...
    def _parse_product_line(self, line_element) -> Product:
        ...
        try:
            ...
            underlying_code = self._get_text(line_element, './/cbc:ID[@schemeID="999"]', "")

            # Unit of measure
            unit_element = line_element.find(".//cbc:InvoicedQuantity", self.namespaces)
            unit_code = unit_element.get("unitCode", "") if unit_element is not None else ""
            ...
            # Unit price
            unit_price_str = self._get_text(line_element, ".//cac:Price/cbc:PriceAmount", "0")
            ...
            if not total_price_str or total_price_str == "0":
                total_price_str = self._get_text(line_element, ".//cbc:LineExtensionAmount", "0")
            ...
            iva_percentage = Decimal(iva_str.replace(",", ".")) if iva_str else Decimal("0")
    ...
    def _get_text(self, element, xpath: str, default: str = "") -> str:
        ...
        try:
            ...
            simple_xpath = xpath.replace("cbc:", "").replace("cac:", "").replace("ext:", "").replace("sts:", "")
    ...
    def _compute_iva_from_amounts(self, line_element) -> Decimal:
        ...
        try:
            ...
            taxable = Decimal(taxable_str.replace(",", ".")) if taxable_str else Decimal("0")
